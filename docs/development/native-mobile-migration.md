...
Start accessibility, localization, restoration, and unsaved drafts in M2, while building each workflow. Match the React Native app's look and interaction on every native screen: navigation (the bottom tab bar with its capture button, the drawer), color tokens including dark mode and user themes, typography, spacing, icons, row and editor layouts. Users receive the native client as an ordinary update, so it must not feel like different software. Verify with side-by-side screenshots of both clients on the same data; a deliberate difference needs explicit maintainer acceptance (maintainer direction, 2026-09-23). Reuse native widgets, capture queues, Siri/Shortcuts, and Watch code; validate their delivery and persistence through the new host. A working foreground engine does not prove extensions or background execution work.
