...
| --- | --- | --- | --- | --- | --- | --- | --- |
| F01 | Open/load/restart; valid empty library; failed/partial reads write nothing | `apps/mobile/lib/storage-adapter.ts` | I (dev DB + RN upgrade path) | N | `apps/android-native/scripts/check-boot-gates.mjs` (checkpoint before first write, validated load, legacy guard before any open) | CPH2655 / Android 16, 2026-09-23, `node apps/android-native/scripts/check-upgrade-device.mjs 44882663`: RN v1.3.2 → native in place shows every row; json-ahead marker, damaged DB (empty and non-empty WAL) and missing DB each fail closed with every file byte-identical | Disposable `.upgradetest` package only; signed-channel upgrades are R01–R03 |
