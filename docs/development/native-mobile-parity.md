...
| --- | --- | --- | --- | --- | --- | --- | --- |
| F01 | Open/load/restart; valid empty library; failed/partial reads write nothing | `apps/mobile/lib/storage-adapter.ts` | I (dev DB only) | N | `apps/android-native/scripts/check-boot-gates.mjs` (checkpoint before first write, validated load, fail-closed boot) | CPH2655 / Android 16, dev package, 2026-09-23: checkpoint replaced by a directory → `Storage unavailable`, commands disabled; `eba75baea` lifecycle run: boot validation after restart | Opens the dev DB, not the RN path; upgrade harness in progress |
