...
- Status icons now mean one thing each. Before, the hourglass meant **Waiting** in the task editor and **Incubate** in inbox processing, **Someday** had a different icon in each place, and on desktop **Start later** and **Someday** shared a clock in one row. The editor and inbox processing now use the same icons as the list navigation (Waiting is a pause circle), **Incubate** has its own seedling icon, and the hourglass is left to **Time estimate**. Desktop and mobile. (#1256)
- Desktop: in the task editor's title, typing `%` now suggests people, and accepting one fills **Assigned To** and removes the token from the title, like `@`, `#`, `+`, `!` and `/` already did. Before, `%Name` stayed in the title as plain text. (#1255)
...
- Mobile: long-pressing a task in **Focus** now also offers **Today**. It sets today as the start date, so the task moves under **Today** without being starred as today's focus. The menu is now titled **Start Date**. The menu now also opens for a task that has a due date, and it only offers start dates on or before that date. (#1252)
...
- Desktop and mobile: the buttons and labels of the app no longer use emoji. They now use the same drawn icons as the rest of the app, so they look the same on every phone and font. This covers inbox processing, the AI suggestion rows, Trash and Archived on mobile, and the check marks and close buttons in mobile settings.
