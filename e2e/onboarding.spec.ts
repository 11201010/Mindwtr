...
test('Getting Started opens real capture, Inbox processing, and Focus', async ({ page }) => {
    ...
    // The seeded project must not suppress the hint.
    const help = page.getByRole('complementary', { name: 'Help: Inbox' });
    await expect(help.getByText(/Something that takes more than one action/)).toBeVisible();
    await page.getByRole('button', { name: /Process Inbox \(3\)/ }).click();
    await expect(page.getByText('Refine the task', { exact: true })).toBeVisible();
    await page.reload();
    await expect(page.getByRole('button', { name: 'Help: Inbox', exact: true })).toHaveAttribute('aria-expanded', 'false');
    ...
    await expect(page.getByRole('complementary', { name: 'Help: Focus' })).toBeVisible();
});
