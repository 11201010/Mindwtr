...
import * as syncSettingsSections from './sync-settings-sections';
import {
...
describe('sync settings section roster', () => {
  it('removes the obsolete interval card while preserving the surrounding sync controls', () => {
    expect(syncSettingsSections).not.toHaveProperty('BackgroundSyncInfoCard');
    expect(syncSettingsSections).toEqual(expect.objectContaining({
      RecoverySnapshotsCard: expect.any(Function),
      SyncLastStatusCard: expect.any(Function),
      SyncPreferencesCard: expect.any(Function),
    }));
  });
});
