...
/**
 * Grouping for the task lists that group by a task attribute rather than by
 * project section. Ported verbatim from apps/mobile/lib/task-group-sections.ts so
 * Archive's native host reads the same groups; that file stays the React Native
 * home for TaskList and Someday until it re-exports this one.
 *
 * Mobile's original note follows. TaskList (Inbox, Done, Reference) and Archive both need
